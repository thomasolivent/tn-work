...
public class UDTEventHandler : MonoBehaviour, IUserDefinedTargetEventHandler
{
    ...
    public void OnNewTrackableSource(TrackableSource trackableSource)
    {
        ...
        {
            SettingRoomTargets(nwm.roomTargets);
        }
        #endregion TN CODE
    }
    ...
    public void SettingRoomTargets(int TargetsCount) 
    {
        if(targets.Count != TargetsCount && PhotonNetwork.connected)
        {
            uiSystem.SwitchScreens(screen);
            targetCreatorText.gameObject.SetActive(true);
            targetCreatorText.text = "Create Target: " + (targets.Count + 1) + " / " + TargetsCount;
        }
